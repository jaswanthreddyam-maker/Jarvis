...
def focus_window(target: str) -> ToolResult:
    ...
    if match is None:
        return ToolResult(success=False, message=f"Could not find a window matching '{target}'.", error="window_not_found")

    _USER32.ShowWindow(match.handle, _SW_RESTORE)
    _USER32.SetForegroundWindow(match.handle)
    return ToolResult(success=True, message=f"Focused '{match.title}'.", data={"window_title": match.title, "handle": match.handle})
...
def close_window(target: str) -> ToolResult:
    ...
    return ToolResult(success=True, message=f"Sent a close request to '{match.title}'.", data={"window_title": match.title, "handle": match.handle})
